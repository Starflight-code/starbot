...
class Report
{
    ...
    public string getAttachmentSummary()
    {
        ...
        { // creates a summary of attached files/embeds for the log
    ...
    public List<MessageAttachment> getSendList(RestUserMessage reportEmbedMessage)
    {
        ...
        if (attached.Count != 0 || embeds.Count != 0)
        {
            ...
            { // echos report message embeds in the log channel
                if (embeds[i].Url != null && embeds[i].Url.Contains("tenor"))
                { // tenor URLs require special handling (not valid embeds but attached as embed)
