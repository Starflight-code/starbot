...
namespace StarBot {
    public class Database {
        Dictionary<ulong, DatabaseObject<string>> guildDatabases; // guildID will find associated DatabaseObject
        Dictionary<string, DatabaseObject<string>> otherDatabases;
        //DatabaseObject data;
        ...
        public Database(DiscordSocketClient client) {
            ...
            foreach (SocketGuild guild in client.Guilds) {
                ...
                guilds.Add(guildID);
                guildDatabases.Add(guildID, new DatabaseObject<string>(guildID));
            }
        ...
        public string? fetchValue(string key, ulong guildID) {
            ...
            guildDatabases.TryGetValue(guildID, out DatabaseObject<string>? targetDatabase);
            if (targetDatabase == null) { return ""; }
            return targetDatabase.get(key);
        }
        public string? fetchValue(string key, string identifier) {
            key = Validation.preProcessValue(key);
            otherDatabases.TryGetValue(identifier, out DatabaseObject<string>? targetDatabase);
            if (targetDatabase == null) { return ""; }
            return targetDatabase.get(key);
        }
        ...
        public async Task iterateValue(string key, ulong guildID, bool updateDB = false) {
            ...
            guildDatabases.TryGetValue(guildID, out DatabaseObject<string>? data);
        ...
        public async Task iterateValue(string key, string identifier, bool updateDB = false) {
            ...
            otherDatabases.TryGetValue(identifier, out DatabaseObject<string>? data);
            data.add(key, (++number).ToString());
            if (updateDB) {
        ...
        public async Task setValue(string key, string value, ulong? guildID, bool updateDB = false) {
            ...
            key = Validation.preProcessValue(key);
            guildDatabases.TryGetValue((ulong)guildID, out DatabaseObject<string>? targetDatabase);
            if (targetDatabase == null) { return; }
